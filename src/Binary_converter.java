...
public class Binary_converter implements TextConverter {
	...
	public void convert_character(char c) {
		System.out.println("Converting a character to its binary code:");
		converted_char = Integer.toBinaryString(c);
	...
	public void convert_paragraph(String text) {
		...
		for(int i=0; i< array_words.length ; i++) {
			...
			// step 2: generate the corresponding binary code of the word and save it in your temporary variable
			for(int j=0 ; j < array_words[i].length(); j++)
				...
				word_generated += Integer.toBinaryString(array_words[i].charAt(j));
				
			// step 3: once the whole word is converted, add your converted word to the list
			converted_parag.add(word_generated);
		}
		
		System.out.println("Converting a text to its binary code:");
